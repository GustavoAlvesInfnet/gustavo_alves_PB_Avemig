...
def informacoes():
    ...
    # permite selecionar order e family
    order = st.selectbox('Selecione uma ordem', df['order'].unique())
    family = st.selectbox('Selecione uma familia', df.loc[df['order'] == order]['family'].unique())
    name = st.selectbox('Selecione um nome científico', df.loc[(df['order'] == order) & (df['family'] == family)]['scientific name'].unique(), index=None)
    ...
    # filtra o dataframe para mostrar apenas as aves da ordem e familia selecionadas
    if name is not None:
        df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name)]
        if extinct:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name) & (df['extinct'] == extinct)]
        else:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name)]
    else:
        df = df.loc[(df['order'] == order) & (df['family'] == family)]
        if extinct:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['extinct'] == extinct)]
        else:
            df = df.loc[(df['order'] == order) & (df['family'] == family)]
...
def informacoes():
    ...
    if 'order' not in st.session_state:
        st.session_state.order = df['order'].unique()[0]

    order = st.selectbox('Selecione uma ordem', df['order'].unique(), index=df['order'].unique().tolist().index(st.session_state.order))
    st.session_state.order = order

    family_options = df.loc[df['order'] == order]['family'].unique()
    family = st.selectbox('Selecione uma família', [''] + family_options.tolist())

    if family != '':
        name_options = df.loc[(df['order'] == order) & (df['family'] == family)]['scientific name'].unique()
        name = st.selectbox('Selecione um nome científico', [''] + name_options.tolist())
    else:
        name = ''
    ...
    if name != '':
        df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name)]
        if extinct:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name) & (df['extinct'] == 1)]
        else:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['scientific name'] == name)]
    elif family != '':
        df = df.loc[(df['order'] == order) & (df['family'] == family)]
        if extinct:
            df = df.loc[(df['order'] == order) & (df['family'] == family) & (df['extinct'] == 1)]
        else:
            df = df.loc[(df['order'] == order) & (df['family'] == family)]
    else:
        df = df.loc[df['order'] == order]
        if extinct:
            df = df.loc[(df['order'] == order) & (df['extinct'] == 1)]
        else:
            df = df.loc[df['order'] == order]
